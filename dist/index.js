...
class DocIndexer {
    ...
    async index() {
        ...
        if (!fs.existsSync(this.docsDir)) {
            ...
            console.error(`[AMRIT MCP] Create the directory and drop .md or .txt files in it.`);
            return;
        }
        const files = await (0, glob_1.glob)("**/*.{md,txt,json}", {
            cwd: this.docsDir,
            absolute: true,
        });
        this.chunks = [];
        ...
        console.error(`[AMRIT MCP] Indexed ${this.chunks.length} chunks from ${files.length} files`);
    }
    ...
    search(query, topK = 5) {
        ...
        const scored = this.chunks.map((chunk) => {
            const text = (chunk.content +
                " " +
                chunk.headings.join(" ")).toLowerCase();
            let score = 0;
            for (const term of queryTerms) {
                ...
                const inHeading = chunk.headings.some((h) => h.toLowerCase().includes(term));
                if (inHeading)
                    score += 3;
            }
            return { chunk, score };
        ...
        tools: [
            {
                ...
                description: "Search AMRIT documentation and architecture notes. Use this to answer questions about AMRIT's codebase, API structure, module design, or any concept documented in the AMRIT knowledge base.",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
                            type: "string",
                            description: "Natural language query, e.g. 'how does beneficiary registration work' or 'Spring Boot service layer conventions'",
                        },
                        topK: {
                            type: "number",
            ...
            },
            {
                ...
                description: "List all indexed AMRIT documentation files. Useful to understand what knowledge is available before searching.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: [],
            ...
            },
            {
                ...
                description: "Get a high-level overview of the AMRIT platform — repos, architecture, tech stack, and how the pieces connect. Call this first when starting any AMRIT-related task.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: [],
            ...
            },
            {
                ...
                description: "Generate a structured JIRA ticket from a plain English description of a feature or bug. Uses AMRIT's ticket conventions.",
                inputSchema: {
                    type: "object",
                    properties: {
                        description: {
                            type: "string",
                            description: "Plain English description of the feature, bug, or task",
                        },
                        type: {
                            type: "string",
                        ...
                        component: {
                            type: "string",
                            description: "AMRIT component this relates to, e.g. 'HWC', '104 helpline', 'MMU', 'TM'",
                        },
                    },
            ...
            {
                name: "reindex_docs",
                description: "Re-scan the docs directory and rebuild the index. Run this after adding new documentation files.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: [],
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "get_amrit_context": {
            ...
            const overviewSearch = indexer.search("AMRIT overview architecture repositories", 3);
...
- **Provider service map:** Links a health worker to their facility and role`;
            const dynamicContext = overviewSearch.length > 0
                ? "\n\n## From indexed docs:\n\n" +
        ...
        case "generate_jira_ticket": {
            ...
            const component = args?.component || "General";
            if (!description) {
                return {
            ...
            const relatedContext = relatedDocs.length > 0
                ? relatedDocs
